...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/flare/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/flare/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/flare/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/flare/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/flare/models.json", base), fmt.Sprintf("%s/api/flare/vendors.json", base)
}
